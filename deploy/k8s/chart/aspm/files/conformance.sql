...
-- ----------------------------------------------------------------------------------------------
-- 3. Forced row-level security, with every exception registered and reasoned.
--
-- The exceptions are still NAMED rather than filtered by a predicate. tenant_id_reservation HAS a
-- tenant_id column — there it is the subject of the row, not its scope — so any predicate clever
-- enough to exclude it excludes the next table for the wrong reason and reports nothing.
--
-- Since V072 the names live in `tenant_isolation_exemption`, written by the migrations that create
-- the tables, and this check reads them through unregistered_unforced_tables(). The literal list
-- this check used to carry named three tables; V015 and V052 had added four more global reference
-- tables, and on 2026-09-11 this check fired on all four against a correct schema. A verification
-- that must be edited by hand to stay true eventually stops being true in one direction or the
-- other, and the other direction is the one that matters.
-- ----------------------------------------------------------------------------------------------
DO $$
DECLARE
    unforced text;
...
BEGIN
    ...
    IF unforced IS NOT NULL THEN
        RAISE EXCEPTION 'row-level security is not FORCEd and no exemption is registered for: %. '
                        'FORCE is what applies the policy to the table OWNER as well, and the owner '
                        'here is the migration role (CON-DAT-012, SEC-TEN-008). A deliberately global '
                        'table is registered in tenant_isolation_exemption with its reason by the '
                        'migration that creates it.', unforced;
    END IF;

    SELECT count(*) INTO registered FROM tenant_isolation_exemption;
    RAISE NOTICE 'forced row-level security: every table except the % registered, reasoned exemptions',
                 registered;
END
$$;
