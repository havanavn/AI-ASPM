...
| `aic.assist.use` | Ask a grounded question about the posture; use drafting help on a record already open (ADR-075) |
| `aic.copilot.use` | Converse with the copilot across the caller's whole scope (ADR-077). **Separate from `aic.assist.use` because the reach differs** — drafting help acts on the record in front of the person, the copilot composes across everything they may see, and a tenant must be able to grant one without the other |
...
**Two corrections recorded here rather than fixed silently.** `aic.assist.use` was introduced by ADR-075 and enforced from the day it shipped, and was never added to this catalogue — so the document listed three AI permissions where the product had four, and a reader composing a role from §5.2 could not have granted the one that gates the ask box. `aic.copilot.use` is added at the same time as it is introduced (ADR-077). The catalogue is what a tenant composes roles from; a permission missing from it is a capability nobody can grant on purpose.
...
|---|---|---|---|---|
| 1.2.0 | 2026-09-13 | Chief Software Architect; Principal Security Architect | Added `aic.assist.use` and `aic.copilot.use` to the §5.2 catalogue. The first is a correction: it shipped with ADR-075 and was enforced from that day without ever appearing here, so this document described three AI permissions where the product had four, and a reader composing a role from §5.2 could not have granted the one that gates the ask box. The second is recorded as it is introduced, with the reason it is not simply the first written into the row: drafting help acts on the record in front of the person and the copilot composes across everything they may see, so a tenant has to be able to grant one without the other. | Pending |
