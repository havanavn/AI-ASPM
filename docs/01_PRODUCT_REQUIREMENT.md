...
| `PRD-ASM-026` | A planned assessment window MAY name the assessor team and the individual assessor expected to perform it, both optional and both changeable until the window is converted; the plan view MUST be readable grouped by organization unit, by assessor team, by assessor, and by calendar month, MUST state per group how many applications are owed a review and have no window planned, and MUST allow any group to be selected to narrow the whole view to it. | A plan that names no one is a wish. The question a head of security asks in a planning review is "who has committed to what", per business unit and per team, and the answer was not recordable: ADR-065 deliberately left the window without an owner so as not to prejudge assignment semantics, and the cost recorded there — the plan cannot say whose fortnight it is — was paid in every review as a spreadsheet kept beside the platform. Naming an expected owner is not assignment (the request, when raised, carries its own assignee under `PRD-ASM-018`) and is therefore optional and a soft reference (ADR-030): a disbanded team leaves a window that says so rather than a window that vanished. The per-group count of due-and-unplanned applications is required as a figure rather than left to be inferred from two other columns, because it is the finding a planning review exists to produce, and a reader who must subtract to find it will not. Grouping and narrowing are one gesture because a figure with no path to the rows it counts cannot be checked (`PRD-ASM-024`). | M | AT | `CAP-2.4`, PP-1, PP-6 |
...
| 1.1.0 | 2026-09-13 | Chief Software Architect, Principal Security Architect | Added `PRD-ASM-026`: a planned window may name its expected team and assessor, and the plan is readable per organization unit, team, person and month with a due-and-unplanned count per group and one-gesture narrowing. Recorded with ADR-076, which marks the "no assignee" consequence of ADR-065 superseded in part. Correction owned here: `PRD-ASM-015` through `PRD-ASM-025` were added on 2026-08-25 with ADR-065 and ADR-066 without a row in this table; this row is the first since 1.0.0 and the omission is recorded rather than backfilled with an invented date. | Pending |
